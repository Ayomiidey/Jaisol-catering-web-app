        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`relative flex flex-col items-center justify-center px-4 py-3 transition-colors ${
                isActive
                  ? 'text-primary'
            ...
            >
              ...
              {item.href === '/order' && cartCount > 0 && (
                <span className="absolute right-2 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground">
                  {cartCount}
                </span>
              )}
...
//   return (
//     <nav className="fixed bottom-0 left-0 right-0 border-t border-border bg-background/95 backdrop-blur lg:hidden">
//       <div className="flex items-center justify-around">
//         {navItems.map((item) => {
//           const isActive = pathname === item.href
//           const Icon = item.icon
